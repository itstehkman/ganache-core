...
var interpreter = require("./repl")
var ReplManager = require("./repl_manager")
...
BlockchainDouble.prototype.initialize = function(accounts, callback) {
  ...
  this.data.initialize(function(err) {
    ...
    self.latestBlock(function(err, block) {
      ...
      if (process.env.EVM_DEBUG_ENABLED) {
        self.vm.on('step', function(state) {
          // DEBUG opcode
          if (state.opcode.opcode === 0x46) {
            console.log('DEBUG')
            self.runRepl.bind(self)
            self.runRepl(state);
          }
        })
      }
...
BlockchainDouble.prototype.processTransactionTrace = function(hash, params, callback) {
  ...
  function step_listener(event, next) {
    ...
    var stack = params.disableStack ? null : event.stack.map(function(item) {
...
BlockchainDouble.prototype.runRepl = function(state) {
  let Repl = new ReplManager(interpreter)

  // TODO: reset VM for repl, and restore it later for continued execution

  Repl.start({
    prompt: "0xd3bu6( PC=" + state.pc + " )> ",
    context: {
      repl: Repl,
      state: state,
      address: Web3.currentContractAddress
    },
...
// @return new EVM without modifying the current VM
BlockchainDouble.prototype.newVM = function() {
  let newVM = new VM({
    // Since you cannot modify the state, we do not have to clone it
    state: self.stateTrie,
    blockchain: {
      getBlock: function(number, done) {
        try {
...
BlockchainDouble.prototype.restoreVM = function() {
  console.log(this)
  this.vm = this.oldVm
  this.oldVm = null
}
