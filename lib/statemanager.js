...
StateManager.prototype.processBlock = function(timestamp, callback) {
  ...
  self.blockchain.processNextBlock(timestamp, function(runtime_error, transactions, vm_output) {
    if (runtime_error && runtime_error instanceof RuntimeError == false) {
      // This is bad. Get out.
      return callback(runtime_error, transactions, vm_output);
    }
    ...
    }, callback(runtime_error, transactions, vm_output));
  });
}
...
StateManager.prototype.processBlocks = function(total_blocks, callback) {
  ...
  async.whilst(function() {
    ...
    self.processBlock(function(err, transactions, vm_output) {
      amount_processed += 1;
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  expectedAddress = expectedAddress && expectedAddress.map(function(a) { return a.toLowerCase() });
